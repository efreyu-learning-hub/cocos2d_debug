...
static ImGuiDebugLayer* _debugLayer = nullptr;
static bool _debugLayerDetached = true;

void ImGuiDebugLayer::initializeLayer() {
    if (_debugLayer) return;
    ...
    e->addCustomEventListener(Director::EVENT_BEFORE_SET_NEXT_SCENE, [&](EventCustom*){
        if (!_debugLayer) {
            if (Director::getInstance()->getRunningScene()) {
                _debugLayer = dynamic_cast<ImGuiDebugLayer*>(Director::getInstance()->getRunningScene()->getChildByName("ImGUILayer"));
            }
            if (!_debugLayer) {
                _debugLayer = ImGuiDebugLayer::create();
            }
        }
        // _debugLayer = dynamic_cast<ImGuiLayer*>(Director::getInstance()->getRunningScene()->getChildByName("ImGUILayer"));
        if (_debugLayer) {
            _debugLayer->retain();
            _debugLayer->removeFromParent();
            _debugLayerDetached = true;
        }
    });
    e->addCustomEventListener(Director::EVENT_AFTER_SET_NEXT_SCENE, [&](EventCustom*){
        if (_debugLayer && _debugLayerDetached) {
            Director::getInstance()->getRunningScene()->addChild(_debugLayer, INT_MAX, "ImGUILayer");
            _debugLayer->release();
            _debugLayerDetached = false;
        }
    });
}

#if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
std::string ImGuiDebugLayer::demangle(const char *name) {
...
std::string ImGuiDebugLayer::demangle(const char* mangled_name)
{
    int status = -4;
...
void ImGuiDebugLayer::drawImgui() {
    // Buttons
...
void ImGuiDebugLayer::showNodeEditor(bool* editorOpened) {
    ...
    renderTree({cocos2d::Director::getInstance()->getRunningScene()});
    ImGui::EndChild();
...
ImRect ImGuiDebugLayer::renderTree(cocos2d::Vector<Node*> n) {
    const ImRect nodeRect = ImRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
    if (lastTarget == nullptr) {
        lastTarget = n.front();
    }
    for (auto& node : n) {
        if (node->getName() == "debugNode" || node->getName() == DebugComponent::DRAW_NODE_NAME)
            continue;
        if (typeid(*node).name() == typeid(ImGuiDebugLayer).name())
            continue;

        ImGui::PushID(node);
        ImGui::AlignTextToFramePadding();
        std::string className = demangle(typeid(*node).name());
        ...
        auto visibleChildCount = std::count_if(node->getChildren().begin(), node->getChildren().end(), [](Node* child) {
            return child->getName() != "debugNode" && child->getName() != DebugComponent::DRAW_NODE_NAME;
        });
        ...
        bool isCamera = dynamic_cast<cocos2d::Camera*>(node) != nullptr;
        bool isMenu = dynamic_cast<cocos2d::Menu*>(node) != nullptr;
        bool isScene = dynamic_cast<cocos2d::Scene*>(node) != nullptr;

        // Visibility toggle button (skip for Camera and Scene)
        if (!isCamera && !isScene) {
            ImGui::SameLine(rightEdge - buttonSize);
            bool isVisible = node->isVisible();
            ImVec4 visColor = isVisible ? ImVec4(0.2f, 0.8f, 0.2f, 1.0f) : ImVec4(0.8f, 0.2f, 0.2f, 1.0f);
            ImGui::PushStyleColor(ImGuiCol_Button, visColor);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(visColor.x + 0.1f, visColor.y + 0.1f, visColor.z + 0.1f, 1.0f));
        ...
        // Debug toggle button (skip for Menu and Camera nodes)
        if (!isMenu && !isCamera) {
            ImGui::SameLine(rightEdge - buttonSize * 2 - spacing);
            bool hasDebug = DebugComponent::isDebugComponent(node);
            ImVec4 dbgColor = hasDebug ? ImVec4(0.2f, 0.8f, 0.2f, 1.0f) : ImVec4(0.8f, 0.2f, 0.2f, 1.0f);
            ImGui::PushStyleColor(ImGuiCol_Button, dbgColor);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(dbgColor.x + 0.1f, dbgColor.y + 0.1f, dbgColor.z + 0.1f, 1.0f));
            if (ImGui::SmallButton(hasDebug ? " D " : " d ")) {
                if (hasDebug) {
                    DebugComponent::removeDebugComponent(node);
                } else {
                    DebugComponent::createDebugComponent(node);
                }
            }
            ImGui::PopStyleColor(2);
            if (ImGui::IsItemHovered()) {
                ImGui::SetTooltip("Toggle debug border");
            }
        }
...
ImRect ImGuiDebugLayer::renderPreferences(Node* node) {
    ImRect prefRect = ImRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax());
...
void ImGuiDebugLayer::debugToggleRow(cocos2d::Node* node) {
    if (!node || !node->isRunning()) {
        return;
    }

    // Menu and Camera do not support debug components
    if (dynamic_cast<cocos2d::Menu*>(node) || dynamic_cast<cocos2d::Camera*>(node)) {
        return;
    }

    ImGui::SetNextItemOpen(true, ImGuiCond_Once);
    if (ImGui::CollapsingHeader("Debug Component")) {
        bool hasDebug = DebugComponent::isDebugComponent(node);
        bool tempHasDebug = hasDebug;
        ImGui::Checkbox("Enable Debug", &hasDebug);
        if (hasDebug != tempHasDebug) {
            if (hasDebug) {
                DebugComponent::createDebugComponent(node);
            } else {
                DebugComponent::removeDebugComponent(node);
        ...
        if (hasDebug) {
            auto nodeColor = DebugComponent::getDebugColor(node);
            float vecColors[4] = { nodeColor.r, nodeColor.g, nodeColor.b, nodeColor.a };
            if (ImGui::ColorEdit4(
                  "Debug Color", vecColors, ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_Float)) {
                cocos2d::Color4F newColor(vecColors[0], vecColors[1], vecColors[2], vecColors[3]);
                DebugComponent::setDebugColor(node, newColor);
            }
        }
    }
}

void ImGuiDebugLayer::resetDebugModules() {
    debugModules.clear();
}

void ImGuiDebugLayer::addDebugModules(const std::pair<std::string, std::function<void()>>& item) {
    debugModules.push_back(item);
}

void ImGuiDebugLayer::initEvents() {
...
bool ImGuiDebugLayer::Combo(const char* label, int* current_item, const std::vector<std::string>& items, int items_count, int height_in_items) {
